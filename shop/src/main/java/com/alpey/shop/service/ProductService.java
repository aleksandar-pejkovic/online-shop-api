...
public class ProductService {
	...
	public Product update(Product product, String oldName) {
		try {
			...
			copyNotNullProperties(product, storedProduct);
	...
	public Product copyNotNullProperties(Product src, Product dest) {
		String barcode = src.getBarcode();
		String name = src.getName();
		String unit = src.getUnit();
		double vatPercentage = src.getVatPercentage();
		double price = src.getPrice();
		
		if(hasValue(barcode))
			dest.setBarcode(barcode);
		if(hasValue(name))
			dest.setName(name);
		if(hasValue(unit))
			dest.setUnit(unit);
		if(hasValue(vatPercentage))
			dest.setVatPercentage(vatPercentage);
		if(hasValue(price))
			dest.setPrice(price);
		dest.calculateVAT();
		return dest;
